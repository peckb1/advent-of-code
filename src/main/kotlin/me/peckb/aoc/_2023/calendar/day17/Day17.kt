...
class Day17 @Inject constructor(
  ...
  class LavaPoolDijkstra(
    ...
    inner class NodeWithCost(val node: Node, val heatLoss: Int) : DijkstraNodeWithCost<Node, Int> {
      override fun compareTo(other: DijkstraNodeWithCost<Node, Int>): Int {
        return heatLoss.compareTo(other.cost())
      }
  ...
  fun partOne(filename: String) = generatorFactory.forFile(filename).read { input ->
    ...
    paths
      .filter { it.key.row == lavaPool.lastIndex && it.key.col == lavaPool[0].lastIndex }
      .minOf { it.value }
  }
